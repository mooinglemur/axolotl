...
public:
  ...
  // Subscriber probes. The overview and stats payloads are O(slots) to
  // build, so the owner skips building them entirely when nothing is
  // listening rather than building and discarding.
  bool HasOverviewClients() const;
  bool HasGraphClients() const;
  bool HasStatsClients() const;
  // True once after a client subscribes to /overview, /graph or /stats.
  // Lets the owner rebuild a payload it skipped while nobody was listening,
  // instead of leaving the new client on a stale cached one.
  bool ConsumeSubscriberAdded() {
    return subscriber_added_.exchange(false, std::memory_order_relaxed);
  }
...
private:
  ...
  std::atomic<bool> subscriber_added_{false};
